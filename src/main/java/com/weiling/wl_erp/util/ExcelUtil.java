...
import com.weiling.wl_erp.Interface.ExcelAnnotation;
...
public class ExcelUtil<T> {
        ...
        public HSSFWorkbook exportExcel(List<T> list,String title,Class className,Integer exportType )throws Exception {
            ...
            //设置行高度
            hssfRow.setHeight((short)380);
            //创建单元格(从0开始)
            HSSFCell hssfCell = hssfRow.createCell((short)0);
            //样式对象
            HSSFCellStyle cellStyle = getCellstyle(hssfWorkbook,(short)300,(short)500);
            //将上面获得的样式对象给对应单元格
            ...
            cellStyle = getCellstyle(hssfWorkbook,(short)270,(short)500);
            generateTitle(exportType,fieldList,columnsize,hssfSheet,hssfRow,cellStyle);
            //组装excel的数据
            cellStyle = getCellstyle(hssfWorkbook,(short)220,(short)500);//设置单元格格式
            generateData(list,fieldList,columnsize,rowindex,hssfSheet,cellStyle);
            /**
             * 第1个参数：从哪一行开始
             * 第2个参数：到哪一行结束
             * 第3个参数：从哪一列开始
             * 第4个参数：到哪一列结束
             */
            hssfSheet.addMergedRegion(new CellRangeAddress(0,0,0,columnsize-1));
            //固定表头(前一个参数代表列 后一个代表行）
     ...
     *
     * */
    private int generateData(List<T> list, List<Field> fieldList, int columnsize, int rowindex, HSSFSheet hssfSheet, HSSFCellStyle cellStyle)throws NoSuchMethodException,IllegalAccessException, InvocationTargetException {
        HSSFCell hssfCell;
        HSSFRow hssfRow;
     ...
     *
     * */
    private void generateTitle(Integer exportType, List<Field> fieldList, int columnsize, HSSFSheet hssfSheet, HSSFRow hssfRow, HSSFCellStyle cellStyle) {
        HSSFCell hssfCell;
        for (int i = 0;i<columnsize;i++){
            Field field = fieldList.get(i);
            if (field.isAnnotationPresent(ExcelAnnotation.class)){
                ...
                hssfCell.setCellStyle(cellStyle);
                hssfSheet.setColumnWidth((short)i,(short)annotation.width());
            }
        }
    }
    ...
    /**
     * 功能:设置excel表格默认格式
     * @param hssfWorkbook 需要导出的表格数据
     * @param i 字体粗度
     * @param i1 表格线的粗度
     * @return
     * */
    private HSSFCellStyle getCellstyle(HSSFWorkbook hssfWorkbook, short i, short i1) {
    ...
    public SXSSFWorkbook exportExcel07S(List list, String title, Class className, int i) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
        // 获取属性
        Field[] fields = className.getDeclaredFields();
        List<Field> fieldList = new ArrayList<Field>();
        for (Field fie : fields) {
        ...
        createTitle07S(i, fieldList, columnsize, hSheet, hRow, cellStyle);
     ...
     * @return
     */
    private CellStyle getCellStyle07S(SXSSFWorkbook hWorkbook, short i, short i1) {
        CellStyle cellStyle;
        Font font;
